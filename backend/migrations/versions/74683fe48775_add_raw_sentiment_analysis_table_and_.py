...
def upgrade() -> None:
    ...
    # Drop old columns if they exist. Add error handling if they might not.
    with op.batch_alter_table('alerts') as batch_op:
        try:
            batch_op.drop_column('alert_type')
        except Exception as e:
            print(f"Could not drop column alert_type from alerts (may not exist): {e}")
        try:
            batch_op.drop_column('last_checked')
        except Exception as e:
            print(f"Could not drop column last_checked from alerts (may not exist): {e}")
        try:
            # If last_triggered was the old name and we are adding last_triggered_at as new
            batch_op.drop_column('last_triggered') 
        except Exception as e:
            print(f"Could not drop column last_triggered from alerts (may not exist or renamed): {e}")
